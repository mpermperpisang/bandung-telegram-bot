...
begin
  client = Mysql2::Client.new(:host => "localhost", :username => "root", :password => "bukalapak")
    ...
    File.open('/home/mpermper/bandung-telegram-bot/require/helper_cap.rb', 'w+') do |f|
    ...
    line = File.read('/home/mpermper/bandung-telegram-bot/require/helper_cap.rb')
    line1 = line.gsub('{"chat_id"=>"', '')
    line2 = line1.gsub("\n", '')
    @chat_id = line2.gsub('"}', '')

    File.open('/home/mpermper/bandung-telegram-bot/require/ruby_cap.rb', 'w+') do |f|
    ...
    line = File.read('/home/mpermper/bandung-telegram-bot/require/ruby_cap.rb')
    ...
    unless @staging == nil || @staging == ""
      ...
      p File.read('/home/mpermper/bandung-telegram-bot/require/ruby_cap.rb') unless File.read('/home/mpermper/bandung-telegram-bot/require/ruby_cap.rb').empty?
      begin
        EnvBash.load("/home/mpermper/bandung-telegram-bot/helper/jenkins/exec_cap_deploy.bash")
      rescue
        EnvBash.load("/home/mpermper/bandung-telegram-bot/helper/jenkins/exec_cap_current.bash")
      end
      ...
      if @staging != "103"
        Net::SSH.start("#{@staging_ip}", "bukalapak", :password => "bukalapak") do |session|
          begin
            session.scp.download! "/home/bukalapak/deploy/log/cap.log", "/home/mpermper/bandung-telegram-bot/log"
          rescue
            session.scp.download! "/home/bukalapak/current/log/cap.log", "/home/mpermper/bandung-telegram-bot/log"
          end
        end

        bot.api.send_document(chat_id: @chat_id, document: Faraday::UploadIO.new('/home/mpermper/bandung-telegram-bot/log/cap.log', 'text/plain'))
        text = File.read('/home/mpermper/bandung-telegram-bot/log/cap.log')
      else
        begin
