...
begin
  client = Mysql2::Client.new(:host => "localhost", :username => "root", :password => "bukalapak")
    ...
    File.open('/home/mpermper/bandung-telegram-bot/require/helper_rake.rb', 'w+') do |f|
    ...
    line = File.read('/home/mpermper/bandung-telegram-bot/require/helper_rake.rb')
    line1 = line.gsub('{"chat_id"=>"', '')
    line2 = line1.gsub("\n", '')
    @chat_id = line2.gsub('"}', '')

    File.open('/home/mpermper/bandung-telegram-bot/require/ruby_rake.rb', 'w+') do |f|
    ...
    line = File.read('/home/mpermper/bandung-telegram-bot/require/ruby_rake.rb')
    ...
    unless @staging == nil || @staging == ""
      ...
      p File.read('/home/mpermper/bandung-telegram-bot/require/ruby_rake.rb') unless File.read('/home/mpermper/bandung-telegram-bot/require/ruby_rake.rb').empty?
      begin
        EnvBash.load("/home/mpermper/bandung-telegram-bot/helper/jenkins/exec_rake_deploy.bash")
      rescue
        EnvBash.load("/home/mpermper/bandung-telegram-bot/helper/jenkins/exec_rake_current.bash")
      end
      ...
      if @staging != "103"
        Net::SSH.start("staging#{@staging}.vm", "bukalapak", :password => "bukalapak") do |session|
          begin
            session.scp.download! "/home/bukalapak/deploy/log/rake.log", "/home/mpermper/bandung-telegram-bot/log"
          rescue
            session.scp.download! "/home/bukalapak/current/log/rake.log", "/home/mpermper/bandung-telegram-bot/log"
          end
        end

        bot.api.send_document(chat_id: @chat_id, document: Faraday::UploadIO.new('/home/mpermper/bandung-telegram-bot/log/rake.log', 'text/plain'))
        text = File.read('/home/mpermper/bandung-telegram-bot/log/rake.log')
      else
        begin
